...
DrawingPad = function(options) {
	...
	*/
	...
	function draw(data, fromMe){
		if(!fromMe && data.type == 'mousedown')console.log("FFHJKFDHJKFHJKLDFSHJKLDFSHJKDFS");
		if(DP.thisObj[data.id]){
			...
			if (data.type === eventType.down) {
				...
				if(fromMe){
					window.socket.emit('drawRequest', data);
				}
	...
	function _buildToolBar(){
		...
		tool="";
		for(i=1; i < len;i+=1) {
			if(i != 2)tool +=  "<li data-toggle='tooltip' data-placement='right' data-original-title='" + tools[i] + "' class='sprite " + tools[i] + "'></li>";
		}
	...
	function _setEventHandlers(){
		...
		$(".toolbar").find(".sprite").click(function(){
			...
			if($(this).hasClass(tools[0])){			//share
				...
				$(".btn-danger").click(function(){
					...
					DP.thisObj.socket.emit("erase",DP.thisObj.id);
				});
				$(".alert").show().alert();
				...
				$.ajax({
				    ...
				    type: 'post',
				    headers: {
				        Authorization: 'Client-ID cbc893bf63e8ed2'
				    },
				    data: {
				        image: img
				    },
				    ...
				    success: function(response) {
				        if(response.success) {
				            // window.location = response.data.link;
										window.socket.emit("file", response.data.link);
				        }
				    }
				});
			}
		...
		DP.thisObj.find(".myCanvas").bind(events, function(e){
			...
			if(DP.isDrawing || DP.isLineDrawing) {
				...
				if(DP.okToDraw || e.type === eventType.up && e.type != eventType.down) {
	 ...
	 * Init DrawingPad
	 */
	this.init = function(selector, id) {
		...
		DP.thisObj.socket = window.socket || io();
		window.socket = window.socket || DP.thisObj.socket;
